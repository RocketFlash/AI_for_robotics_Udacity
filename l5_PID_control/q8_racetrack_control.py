...
############## ONLY ADD / MODIFY CODE BELOW THIS LINE ####################
    ...
    def cte(self, radius):
        ...
        yc = self.y
        # print xc, yc
        # determine which portion

        # if y is greater less or equal to the radius
        # Portion 1
        # circle
        ...
        if xc <= radius:
            ...
            d_to_bot = sqrt(pow((center[0] - xc), 2) + pow((center[1] - yc), 2))
            cte = -1 * (radius - d_to_bot)

        # square
        elif xc > radius and xc <= 3 * radius:
            # square
            # print 'square'
            if (yc >= 0 and yc <= 2 * radius):
                if (yc >= radius):
                    cte = -1 * (2 * radius - yc)
                else:
                    cte = -1 * yc
