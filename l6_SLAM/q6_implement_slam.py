...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    omegax = [[0.0 for row in range(omega_size)] for col in range(omega_size)]
    omegay = [[0.0 for row in range(omega_size)] for col in range(omega_size)]
    xix = [[0.0] for col in range(omega_size)]
    xiy = [[0.0] for col in range(omega_size)]
    omegax[0][0] += 1.
    omegay[0][0] += 1.
    xix[0][0] += 50.
    xiy[0][0] += 50.
    for locI in range(len(data)):
        for measI in range(len(data[locI][0])):
            ...
            msx = data[locI][0][measI][1] * measurement_noise
            msy = data[locI][0][measI][2] * measurement_noise
            omegax[locI][locI] += measurement_noise
            omegax[lmI][lmI] += measurement_noise
            omegax[locI][lmI] += -measurement_noise
            omegax[lmI][locI] += -measurement_noise
            omegay[locI][locI] += measurement_noise
            omegay[lmI][lmI] += measurement_noise
            omegay[locI][lmI] += -measurement_noise
            omegay[lmI][locI] += -measurement_noise
            xix[locI][0] += -msx
            xix[lmI][0] += msx
            xiy[locI][0] += -msy
            xiy[lmI][0] += msy
        dx = data[locI][1][0] * motion_noise
        dy = data[locI][1][1] * motion_noise
        omegax[locI][locI] += motion_noise
        omegax[locI+1][locI+1] += motion_noise
        omegax[locI+1][locI] += -motion_noise
        omegax[locI][locI+1] += -motion_noise
        omegay[locI][locI] += motion_noise
        omegay[locI+1][locI+1] += motion_noise
        omegay[locI][locI+1] += -motion_noise
        omegay[locI+1][locI] += -motion_noise
        xix[locI][0] += -dx
        xix[locI+1][0] += dx
        xiy[locI][0] += -dy
        xiy[locI+1][0] += dy
