    ...
    while True:
        ...
        for i in range(0, len(newpath)):
            for j in range(len(newpath[0])):
                ...
                total_error += abs(old_val - newpath[i][j])
...
smooth_path = smooth(testpath1)
# printpaths(path, smooth_path)
plt.plot([x[0] for x in testpath1], [x[1] for x in testpath1], 'b')
plt.plot([x[0] for x in smooth_path], [x[1] for x in smooth_path], 'k')
plt.show()
