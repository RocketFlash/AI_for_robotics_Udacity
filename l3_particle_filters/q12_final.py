...
class robot:
    ...
    def move(self, motionv):  # Do not change the name of this function

        # ADD CODE HERE
        distance = random.gauss(motionv[1], self.distance_noise)
        steering_angle = random.gauss(motionv[0], self.steering_noise)
        ...
        if abs(beta) >= 0.001:
            ...
            x_prime = cx + sin(self.orientation + beta) * R
            y_prime = cy - cos(self.orientation + beta) * R
            orientation_prime = (self.orientation + beta) % (2 * pi)
        else:
            x_prime = self.x + (distance * cos(self.orientation))
            y_prime = self.y + (distance * sin(self.orientation))
            orientation_prime = self.orientation % (2 * pi)

        result = robot(self.length)
        result.set(x_prime, y_prime, orientation_prime)
        result.set_noise(bearing_noise, steering_noise, distance_noise)

        return result  # make sure your move function returns an instance
        # of the robot class with the correct coordinates.

    def sense(self, mode=1):  # do not change the name of this function
        Z = []
